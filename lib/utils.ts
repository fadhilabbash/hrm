...
export const geMaritalStatus = (value: string) => {
  const maritalStatus = MaritalStatus.find((item) => item.value === value);
  return maritalStatus ? maritalStatus.label : "غير معروف";
};
export const getEducationGrade = (value: string) => {
  const educationGrade = EducationGrade.find((item) => item.value === value);
...
export const getEmployeeType = (value: string) => {
  const employmentType = EmploymentType.find((item) => item.value === value);
